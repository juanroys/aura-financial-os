...
import { DollarSign, ArrowUpRight, ArrowDownRight, Plus, Calendar, PieChart, ShieldCheck, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="flex flex-col relative w-full font-jakarta">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm sm:text-base font-extrabold text-white tracking-tight font-jakarta">
              Control de Ingresos & Gastos
            </h3>
            <p className="text-[10px] text-gray-300 font-medium">Flujo de Caja Startup & Personal</p>
          </div>
        </div>

        <div className="flex items-center gap-2">
          <button className="hidden sm:flex px-4 py-2 rounded-full bg-white/10 text-white text-xs font-bold hover:bg-white/20 transition-all items-center gap-1.5 border border-white/15">
      ...
      {!isCollapsed && (
        <div className="interlock-white-body p-4 sm:p-7 space-y-6 animate-fadeIn">
          
          {/* KPI Cards Summary */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 sm:p-5 rounded-2xl bg-gray-50 border border-gray-200/80 space-y-2">
              ...
              <p className="text-xl sm:text-2xl font-black text-[#101217]">${totalIncome.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
            ...
            <div className="p-4 sm:p-5 rounded-2xl bg-gray-50 border border-gray-200/80 space-y-2">
              ...
              <p className="text-xl sm:text-2xl font-black text-[#101217]">${totalExpenses.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
              <span className="text-[10px] text-gray-400 font-bold">Burn Rate: ${healthMetrics.burnRateMonthly}/mes</span>
            </div>

            <div className="p-4 sm:p-5 rounded-2xl bg-[#101217] text-white space-y-2 shadow-md">
              ...
              <p className="text-xl sm:text-2xl font-black text-[#d6f535]">${netCashFlow.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
              <span className="text-[10px] text-gray-300 font-medium">Salud: {healthMetrics.stabilityTier} ({healthMetrics.score}/100)</span>
            </div>
          </div>

          {/* Quick Add Form */}
          <form onSubmit={handleSubmit} className="p-4 sm:p-5 rounded-2xl bg-gray-50/80 border border-gray-200/80 space-y-4">
            <h4 className="text-xs font-extrabold text-[#101217] uppercase tracking-wider flex items-center gap-1.5">
              <Plus className="w-4 h-4 text-[#10d670]" /> Registrar Movimiento
            </h4>

            <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
              <input
                type="text"
                placeholder="Descripción (ej: Servidores, Sueldo)"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                className="md:col-span-2 px-4 py-3 rounded-xl bg-white border border-gray-300 text-xs text-gray-800 placeholder-gray-400 focus:outline-none focus:border-[#101217]"
              />
              ...
              <input
                ...
                className="px-4 py-3 rounded-xl bg-white border border-gray-300 text-xs text-gray-800 placeholder-gray-400 focus:outline-none focus:border-[#101217]"
              />

              <select
                value={type}
                onChange={(e) => setType(e.target.value as 'income' | 'expense')}
                className="px-4 py-3 rounded-xl bg-white border border-gray-300 text-xs text-gray-800 focus:outline-none focus:border-[#101217]"
              >
              ...
              <button
                type="submit"
                className="px-5 py-3 rounded-xl bg-[#101217] text-white font-bold text-xs hover:bg-black transition-all shadow flex items-center justify-center gap-1.5"
              >
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between border-b border-gray-100 pb-2">
              <h4 className="text-xs font-extrabold text-[#101217] flex items-center gap-2">
                <PieChart className="w-4 h-4 text-gray-500" /> Transacciones Recientes
              </h4>
            </div>
              ...
              <table className="w-full text-left text-xs font-jakarta">
                <thead>
                  <tr className="text-[10px] text-gray-400 uppercase border-b border-gray-100 font-semibold">
                    ...
                    <th className="py-2.5 hidden sm:table-cell">Categoría</th>
                    <th className="py-2.5 hidden sm:table-cell">Deducible</th>
